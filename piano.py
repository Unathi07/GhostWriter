    ...
    html = f"""
            ...
            <script>
                ...
                whiteKeys.forEach(note => {{
                    ...
                    key.style.backgroundColor = activeNotes.includes(note) ? "lightgreen" : "white";
                ...
                blackKeys.forEach(({{note, left}}) => {{
                    ...
                    key.style.zIndex = "1";
                    key.style.backgroundColor = activeNotes.includes(note) ? "lightgreen" : "black";
                    piano.appendChild(key);
                }});
                ...
                const stopChord = () => {{
                    if (!window.chordOscs || !window.chordCtx) {{
                        return;
                    }}
                ...
                btn.addEventListener("touchstart", playChord);
                btn.addEventListener("touchend", stopChord);

                if ({str(autoplay).lower()}) {{
                    playChord();
                    setTimeout(stopChord, 300);
                }}
            </script>
            """
    return html
